...
@Component
@Slf4j
@Order(Ordered.HIGHEST_PRECEDENCE) //设置MetricsAspect最高优先级
public class MetricsAspect {
	...
	@Pointcut("@annotation(com.learning.aop.annotation.Metrics)")
	// @Pointcut("within(@com.learning.aop.annotation.Metrics *)")
	...
	public Object metrics(ProceedingJoinPoint pjp) throws Throwable {
		 ...
		 * 在这种情况下方法上必然是没有@Metrics注解的，
		 * 我们需要获取一个默认注解。
		 * 虽然可以手动实例化一个@Metrics注解的实例出来，
		 * 但为了节省代码行数，我们通过在一个内部类上定义@Metrics注解方式，然后通过反射获取注解的小技巧，
		 * 来获得一个默认的@Metrics注解的实例
		 */
		if (metrics == null) {
			@Metrics
			final class c {
